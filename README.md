...
1. Открыл терминал и создал новый проект Laravel с именем `MyApp` с помощью Composer:
   ```bash
   composer create-project laravel/laravel:10 MyApp
   ```
2. Перешел в директорию проекта:
   ```bash
   cd MyApp
   ```
3. Запуcтил встроенный сервер Laravel:
   ```bash
   php artisan serve
   ```
...
2. Сгенерируйте ключ приложения, который будет использоваться для шифрования данных:
   ...
   **Вопрос**: Что будет, если данный ключ попадет в руки злоумышленника?
    ...
    - **Ответ**: Этот ключ используется для шифрования сессий, куки и других данных. Если злоумышленник получит доступ к этому ключу, он сможет расшифровывать и читать зашифрованные данные, что ставит под угрозу конфиденциальность пользователей.
...
6. Вместо ручного создания маршрутов для каждого метода можно использовать **ресурсный контроллер**, который автоматически создаст маршруты для всех **CRUD-операций**:
    ...
    - Ресурсный маршрут лучше подходит для ресурсного контроллера

      | HTTP Метод |          URI          |  Метод контроллера  |     Имя маршрута     |
      |:----------:|:---------------------:|:-------------------:|:--------------------:|
